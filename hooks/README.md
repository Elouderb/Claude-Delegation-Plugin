...
```text
hooks/hooks.json               ← GENERATED by the installer (git-ignored, not committed)
templates/hooks.json.tmpl      ← committed source for hooks.json
...
`hooks/hooks.json` is **generated** from `templates/hooks.json.tmpl` by the
bootstrap installer (`installer/install.sh` / `installer/install.ps1`), which
substitutes the venv interpreter's absolute path; it is git-ignored, so only the
template is committed. Hook commands use `${CLAUDE_PLUGIN_ROOT}` for every
non-interpreter path so the package stays portable. Run the installer to
(re)generate it — a **fresh clone has no `hooks/hooks.json`** until you do, and
the hooks simply do not fire until then (silent by design, not an error). Because
it is now a generated artifact, the plugin's own PreToolUse guard blocks direct
edits to it inside this repo — change the template instead.
