...
--     CORRECTION (card 4d362ffb, post-migration, comment-only -- no DDL here
--     changed): the single-cast claim above was verified only against a
--     short/clean synthetic vector and FAILS SQLSTATE 22018 on a realistic
--     ~8KB full-precision embedding (all 759 documents of the live corpus
--     ETL hit this). services/api/memory_store.py::MemoryStore._insert_chunks
--     now uses CAST(CAST(? AS NVARCHAR(MAX)) AS VECTOR(n)) instead.
