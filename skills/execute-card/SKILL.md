...
4. Select the agent: `implementer` for scoped application work, `complex-implementer` for difficult or repo-wide changes (cross-module, large refactors, shared or central code), `database-engineer` for DB work, or `research-planner` when choices remain unresolved.
