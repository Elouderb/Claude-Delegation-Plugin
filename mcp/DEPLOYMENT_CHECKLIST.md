...
- [x] No hardcoded absolute paths in committed source
- [x] `.mcp.json` / `hooks/hooks.json` are GENERATED by the installer from
      `templates/` (git-ignored; interpreter path substituted per machine,
      `${CLAUDE_PLUGIN_ROOT}` used for every other path)
...
├── .mcp.json                    ← MCP server config (GENERATED by installer; git-ignored)
├── hooks/
│   ├── hooks.json               ← Graph-sync + file-protection hooks (GENERATED; git-ignored)
│   └── README.md
├── scripts/                     ← Hook implementations
├── agents/                      ← 10 delegation agents
├── skills/                      ← workflow skills
├── templates/                   ← Card/workflow + launcher-config templates (mcp.json.tmpl, hooks.json.tmpl)
├── installer/                   ← Bootstrap installers (install.sh, install.ps1, verify_install.py)
└── mcp/
    ├── server.py                ← Thin MCP entrypoint (registers the full tool set)
...
```
Creates the plugin venv, installs core deps (plus any selected extras), GENERATES
`.mcp.json` + `hooks/hooks.json` with this machine's interpreter path, and verifies
the result with a real MCP handshake.
...
```
The manifest, MCP server, hooks, agents, and skills are auto-discovered. The
launcher config is generated (not committed), so there is no per-machine path to
edit — but the installer must have run first (a fresh clone registers nothing
until it does).
...
1. **Installer Completes Cleanly**
   ```bash
   installer/install.sh   # venv + deps + generated config + verified handshake
   ```

2. **Server Starts Without Errors**
   ```bash
   .venv/bin/python mcp/server.py   # initializes database, logs to stderr, serves over stdio
   ```

3. **Tests Pass**
   ```bash
   .venv/bin/python mcp/test_server.py   # all 8 card tests should pass
   ```
